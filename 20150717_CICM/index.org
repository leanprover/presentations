...
- Microsoft Research works actively in both of them
...
- We can optionally have =Prop= the type of all propositions
...
nat.zero  : nat
nat.succ  : nat → nat
nat.rec   : Π (C : nat → Type),
   C nat.zero →
   (Π (a : nat), C a → C (nat.succ a)) →
   n → C n
...
* Ad-hoc polymorphism
  ...
  - We can *override* ad-hoc polymorphism and overloading
...
- [[https://github.com/htzh/leanproved][Available on github]]
...
theorem first_sylow_theorem : ∀ p, prime p →
∀ n, p^n ∣ card(A) → ∃ (H : finset A) (sg : is_finsubg H), card(H) = p^n
#+END_SRC
